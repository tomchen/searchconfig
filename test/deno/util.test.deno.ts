import { autoDetectLoader } from '../../deno/util.ts'
import {
  getConfig,
...
describe('autoDetectLoader', () => {
  ...
  test('after registry.addExt() & .reset()', () => {
...
describe('defaultConfigGetStrategy', () => {
  describe('function', () => {
    ...
    test('hasYaml and fromDir option', () => {
      const stra = defaultConfigGetStrategy('johndoe', {
        hasYaml: true,
    ...
    test('before (array) & hasYaml option', () => {
      const stra = defaultConfigGetStrategy('johndoe', {
        before: [
          {
            filepath: 'special.config.json',
